...
1. 检查已安装的内核版本。
  ...
  如果我们现在进入 [Linux 内核官网][GlobalLink-2.1.1.3-2]，在撰写本文时，我们看到最新的内核版本是 `4.10.1`（其他版本可以从同一网站获得）。
...
2. 在 `CentOS 7` 中升级内核。
  ...
  但是，这只会升级内核到仓库中可用的最新版本 - 而不是在 [Linux 内核官网][GlobalLink-2.1.1.3-2] 中可用的最新版本。不幸的是，`Red Hat` 只允许使用前者升级内核。
...
via：[http://www.tecmint.com/install-upgrade-kernel-version-in-centos-7/](http://www.tecmint.com/install-upgrade-kernel-version-in-centos-7/)

作者：[Matei Cezar](http://www.tecmint.com/author/cezarmatei/) 译者：[geekpi](https://github.com/geekpi) 校对：[wxy](https://github.com/wxy)

本文由 [LCTT](https://github.com/LCTT/TranslateProject) 原创编译，[Linux中国][GlobalLink-2.1.1.3-1] 荣誉推出

[GlobalLink-2.1.1.3-1]: https://linux.cn/article-8310-1.html
...
7. 将刚才初始化的分区加入到虚拟卷组名中。
  ...
  !> 输入命令出现 `Couldn't create temporary archive name.`，[CSDN](https://blog.csdn.net/weixin_41772761/article/details/102662739) 上的文章解释说，因为存储空间使用 `100%`，无法挂载，须预留部分空间出来，所以删除 `Python` 安装文件。
...
7. 使环境变量配置生效。
  ...
  查看 [CSDN](https://blog.csdn.net/blueheart20/article/details/72827666) 文章得知，缺少 `zlib` 解压缩类库。
